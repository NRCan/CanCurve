...
- DDFP is an Xactimate + Excel based workflow. expert-oriented with a focus on reproducing Xactimate data.
- CanCurve is python-based QGIS plugin. user-oriented with a focus on usability.
...
DDFP assigns these to 'split', which is typically then evenly divided between the basement and the main floor (behavior is customized on the 'DDFwrk_grp2' tab). 

When using CanCurve's `misc.port_estimate.ddfp_inputs_to_ci` to extract Cost Information data from an Xactimate raw estimate file, entries without a matching `group_code` are assigned to storey=0. 

# Difference evaluation
To quantitatively evaluate the difference in resulting depth-damage functions between DDFP and CanCurve, a set of 59 scenarios from 2 study areas were computed.
...
- **p01_extract_DDFP**: extract data into CanCurve format (CostItem csv, fixed costs, metadata) and extract the DDFP supplied CanFlood fromat ddf
